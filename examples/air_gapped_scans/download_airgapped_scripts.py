"""Download latest scanning scripts for Cyberwatch air gapped scans"""
...
def download_scripts(parsed_args, scripts, client):
    ...
    print("INFO: Script saved in {}".format(os.path.join(os.path.abspath(os.path.dirname(__file__)), 'Scripts')))
...
def launch_script(parsed_args):
    '''Launch script'''
    client = connect_api()
    scripts = client.fetch_airgapped_scripts()
    clean_directory()
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Script using Cyberwatch API to get the latest scanning scripts for Cyberwatch air-gapped scans")
